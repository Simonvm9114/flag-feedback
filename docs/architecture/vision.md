...
The **flag-feedback** package remains a **framework-agnostic, zero-runtime-dependency** browser library (see `docs/constitution.md`, Principle 1). Host developers own branding (activator), access control, and the feedback receiver. The package continues to own the feedback UX (panel, targeting, recording) and the **versioned feedback package contract** optimized for AI agent consumption (see `docs/functional/context.md`).

Core shape is stable: `initFeedback` → in-browser session (MVP: `sessionStorage` draft persistence per `docs/architecture/decisions/data.md`) → single POST to a host-configured endpoint. Shadow DOM encapsulation for overlay UI remains the default pattern for style isolation.
...
| **Persistence**          | MVP ships tab-scoped `sessionStorage` drafts (see `docs/architecture/decisions/data.md`); post-MVP: cross-tab or `localStorage` only if product requires it |
