You are building Batch 6 — Idle behaviour and draft persistence of the MVP. The stories in this batch are: US-18, US-19. The done definition for this batch is `requirements/mvp/batch-6-acceptance-criteria.md`.

**Phase 6.1 — Read context**

Before taking any other action, update `CLAUDE.md` to record this command as active: if a `## Active Command` section already exists, replace it and its preceding `---` separator; otherwise append a new `## Active Command` section (preceded by `---`) stating that `/project:build-batch-6` is currently running. Only one active command may appear in `CLAUDE.md` at a time.

Read the following before writing any code. Where documents conflict, higher entries override lower ones:

1. `docs/constitution.md`
2. `requirements/mvp/batch-6-acceptance-criteria.md` — the done definition for this batch
...
Then verify each criterion in `requirements/mvp/batch-6-acceptance-criteria.md`. For each one, determine whether it is met by the current implementation. If any criterion is not met, address it now.

Also verify the implementation quality dimensions identified in Phase 6.1: confirm that Phase 2 content and branding requirements are correctly applied where relevant to this batch, that the implementation heuristics are respected, and that no architecture decision is violated.

Phase 6.3 is complete when formal checks pass, every criterion in `requirements/mvp/batch-6-acceptance-criteria.md` is met, and the implementation quality checks pass.

**Phase 6.4 — Refinement loop**
...
- [ ] All acceptance criteria in `requirements/mvp/batch-6-acceptance-criteria.md` are met
- [ ] Formal checks pass
