...
**Feedback schema as a contract:** The JSON payload the package POSTs to the configured endpoint is effectively a contract between the package and the receiving system (typically a backend controlled by Simon). Changes to the payload schema should be treated as breaking changes. The schema must remain machine-readable and consistent so that AI agents processing submissions do not need to handle multiple payload shapes. The interaction log within that payload is intended as a **meaningful reconstruction** of host-app activity — not a raw event firehose — so agents can trace where a feature or bug lives (see `docs/architecture/decisions/data.md`).

**Draft persistence:** In-progress feedback (including active recording) is persisted in `sessionStorage` for the browser tab so navigation and reload do not discard work before submit (see `requirements/mvp/user-stories.md`, US-19). Submission still requires network access to the configured endpoint; shared databases are reached via host backends (see `docs/architecture/decisions/backend.md`, Host integration patterns).
